...
import { launchBrowser } from "@/lib/certificates/pdf";
...
export async function POST(request: NextRequest) {
  if (!isAuthorized(request)) {
    console.error("Rejected /api/jobs/process-issuance request: missing or invalid bearer auth");
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL;
  if (!baseUrl) {
    console.error("NEXT_PUBLIC_SITE_URL is not configured; refusing to issue certificates");
  ...
  if (claimError) {
    console.error(`Failed to claim issuance jobs: ${claimError.message}`);
  ...
  const browser = jobs.length > 0 ? await launchBrowser() : null;
    ...
    for (const job of jobs) {
      try {
        ...
        if (!existing) {
          ...
          const verifyUrl = `${baseUrl}/verify/${publicId}`;
